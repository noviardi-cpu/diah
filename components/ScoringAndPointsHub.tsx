...
const ScoringAndPointsHub: React.FC<Props> = ({ analysis, onAnalyzeRequest, initialSyndromeId, isAnalyzing = false, onBack, patientContext }) => {
  ...
  return (
    <div className="flex h-full p-4 gap-4 print:p-0">
      ...
      {/* List Analysis */}
      <div className="w-1/3 min-h-full bg-pink-900 border border-pink-400 rounded-3xl p-5 shadow-2xl flex flex-col">
              ...
              <div 
                ...
                className={`p-4 rounded-2xl cursor-pointer transition-all border ${
                  selectedSyndrome?.syndrome.id === res.syndrome.id 
                    ...
                    : 'bg-pink-950 border-pink-400 hover:bg-pink-800'
                }`}
              >
                ...
                <p className="text-[10px] text-white font-bold uppercase tracking-widest">{res.syndrome.primary_organs?.join(' & ')}</p>
              </div>
            )) : <p className="text-white text-xs italic text-center py-10">Data pasien diperlukan.</p>}
        </div>
      ...
      {/* Detail & Map */}
      <div ref={contentRef} className="flex-1 bg-pink-900 border border-pink-400 rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden">
        {selectedSyndrome ? (
          <>
            <div className="flex justify-between items-start mb-6">
                <div>
                   <h3 className="text-2xl font-black text-white tracking-tighter">{selectedSyndrome.syndrome.name_id}</h3>
                   <p className="text-xs text-white font-bold uppercase tracking-widest">{selectedSyndrome.syndrome.name_en}</p>
                </div>
            ...
            <div className="flex-1 grid grid-cols-2 gap-6 min-h-0">
               <div className="bg-pink-950 border border-pink-400 rounded-3xl overflow-hidden relative shadow-inner">
                  <BodyMapSekarangJadi points={activePoints} />
               </div>
               
               <div className="flex flex-col gap-4 overflow-y-auto pr-2 scrollbar-hide">
                  <div className="bg-pink-950/50 border border-pink-400 p-5 rounded-2xl">
                     ...
                     <div className="flex flex-wrap gap-2">
                        {activePoints.map((p, i) => (
                          <div key={i} className={`px-2 py-1 rounded-lg border text-[10px] font-black flex items-center gap-2 ${p.role === 'manual' ? 'bg-indigo-950 border-indigo-500 text-indigo-300' : 'bg-pink-900 border-pink-400 text-white'}`}>
                             {p.code}
                     ...
                     <form onSubmit={handleAddManualPoint} className="mt-4 flex gap-2">
                        <input value={manualInput} onChange={e => setManualInput(e.target.value)} placeholder="Add point..." className="flex-1 bg-pink-600 border border-pink-400 rounded-xl px-3 py-1.5 text-xs text-white outline-none focus:border-indigo-500 uppercase" />
                        <button type="submit" className="p-2 bg-pink-800 rounded-xl border border-pink-400 text-white"><Plus className="w-4 h-4"/></button>
                     </form>
                  </div>
                  ...
                  {selectedSyndrome.herbal_recommendation && (
                    <div className="bg-emerald-950/10 border border-emerald-500/20 p-5 rounded-2xl">
                       ...
                       <p className="text-[10px] text-white leading-relaxed italic">{selectedSyndrome.herbal_recommendation.chief.join(', ')}</p>
                    </div>
                  )}
               </div>
            </div>
