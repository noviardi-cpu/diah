...
const StrokeEducationPanel: React.FC = () => {
  ...
  const TabButton = ({ id, label, icon }: { id: string, label: string, icon: React.ReactNode }) => (
    <button
      ...
      className={`flex items-center gap-2 px-4 py-3 rounded-xl transition-all font-medium text-xs md:text-sm w-full md:w-auto shrink-0 ${
        ...
        : 'bg-pink-800 text-white hover:bg-pink-700 hover:text-white'
      }`}
    >
      {icon}
      <span>{label}</span>
  ...
  return (
    <div className="h-full bg-pink-950 text-white flex flex-col font-sans overflow-hidden">
      {/* Header */}
      <div className="p-6 border-b border-pink-400 bg-pink-900/50">
        ...
        <div className="text-[10px] text-white mt-1 uppercase tracking-widest">
          Sumber: dr. Michael Wicaksono, M.Med
        </div>
      </div>

      {/* Navigation */}
      <div className="p-4 border-b border-pink-400 overflow-x-auto scrollbar-hide">
      ...
      {/* Content Area */}
      <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-pink-950">
        
        {/* TAB: INTRO */}
        {activeTab === 'intro' && (
          <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
            <div className="bg-pink-900 border border-pink-400 rounded-xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                <div className="bg-indigo-900/20 p-4 rounded-lg border border-indigo-500/30">
                  ...
                  <p className="text-sm text-white">Proses patologis dianggap hanya menyerang <strong>Meridian</strong>.</p>
                </div>
                <div className="bg-rose-900/20 p-4 rounded-lg border border-rose-500/30">
                  ...
                  <p className="text-sm text-white">Proses patologis dianggap menyerang <strong>Organ Zang-Fu</strong> (lebih berat).</p>
                </div>
              </div>
            </div>

            <div className="bg-pink-900 border border-pink-400 rounded-xl p-6">
        ...
        {activeTab === 'etiology' && (
          <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
            {protocols.etiologies.map((item: any, idx: number) => (
              <div key={idx} className="bg-pink-900 border border-pink-400 rounded-xl p-5 hover:border-tcm-primary transition-colors group">
                <h3 className="font-bold text-lg text-white mb-1 group-hover:text-tcm-primary transition-colors">{item.name}</h3>
                <p className="text-xs text-white uppercase tracking-wider mb-4 font-semibold">{item.pathogen}</p>
                
                <div className="bg-pink-950 p-3 rounded-lg border border-pink-400">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.points.map((p: string) => (
                      <span key={p} className="text-xs px-2 py-1 rounded bg-pink-800 text-white border border-pink-400">
                        {p}
                      </span>
                    ))}
                  </div>
          ...
          <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {protocols.scalp_zones.map((zone: any, idx: number) => (
                 <div key={idx} className="bg-pink-900 border border-pink-400 p-4 rounded-xl flex flex-col">
                    <h4 className="font-bold text-white mb-2 border-b border-pink-400 pb-2">{zone.name}</h4>
                    <p className="text-sm text-white leading-relaxed">{zone.desc}</p>
                 </div>
               ))}
            </div>
            
            {/* Additional visual explanation based on slides */}
            <div className="mt-6 p-4 bg-pink-800/50 rounded-xl border border-pink-400">
               <h4 className="font-bold text-white mb-2 text-sm">Zona Motorik (Homunculus)</h4>
               ...
               <p className="text-xs text-white mt-2 italic">Penusukan horizontal 1.5 cun untuk menjangkau beberapa sub-zona.</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'jin' && (
          <div className="max-w-5xl mx-auto animate-fade-in">
             <div className="mb-6">
                ...
                <p className="text-sm text-white">Teknik Profesor Jin Rui (Guangzhou, 1980) menggunakan kombinasi 3 titik efektif.</p>
             </div>

             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {protocols.jins_three_needles.map((group: any, idx: number) => (
                   <div key={idx} className="bg-pink-900 border border-pink-400 rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
                      <div className="bg-pink-800/80 px-4 py-3 border-b border-pink-400 flex justify-between items-center">
                         <h4 className="font-bold text-tcm-primary text-sm">{group.name}</h4>
                         <Zap className="w-3 h-3 text-white" />
                      </div>
                      <div className="p-4 space-y-3">
                         <div>
                            <span className="text-[10px] text-white uppercase font-bold tracking-wider">Titik</span>
                            <ul className="text-sm text-white mt-1 list-disc list-inside">
                               {group.points.map((p: string, i: number) => <li key={i}>{p}</li>)}
                            </ul>
                         </div>
                         <div>
                            <span className="text-[10px] text-white uppercase font-bold tracking-wider">Indikasi</span>
                            <p className="text-xs text-white mt-1 leading-relaxed">{group.indication}</p>
                         </div>
                         {group.method && (
                            <div className="bg-pink-950 p-2 rounded border border-pink-400 text-[10px] text-white italic">
                               Metode: {group.method}
                            </div>
                         )}
                      </div>
        ...
        {activeTab === 'abdominal' && protocols.abdominal_acupuncture && (
           <div className="max-w-4xl mx-auto animate-fade-in">
              <div className="bg-pink-900 border border-pink-400 rounded-xl p-6 shadow-sm mb-6">
                 <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
                    <Activity className="text-tcm-primary" /> {protocols.abdominal_acupuncture.name}
                 </h3>
                 <p className="text-sm text-white mb-4">{protocols.abdominal_acupuncture.indication}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 <div className="bg-pink-900 p-4 rounded-xl border border-pink-400">
                    <h4 className="font-bold text-white text-sm mb-3 border-b border-pink-400 pb-2">Titik Utama (Ren Mai)</h4>
                 ...
                 <div className="bg-pink-900 p-4 rounded-xl border border-pink-400">
                    <h4 className="font-bold text-rose-400 text-sm mb-3 border-b border-pink-400 pb-2">Sisi Sakit (Lumpuh)</h4>
                    <ul className="text-sm text-white space-y-2 list-disc list-inside">
                       {protocols.abdominal_acupuncture.points_affected_side.map((p: string, i: number) => <li key={i}>{p}</li>)}
                    </ul>
                 </div>
                 <div className="bg-pink-900 p-4 rounded-xl border border-pink-400">
                    <h4 className="font-bold text-emerald-400 text-sm mb-3 border-b border-pink-400 pb-2">Sisi Sehat</h4>
                    <ul className="text-sm text-white space-y-2 list-disc list-inside">
        ...
        {activeTab === 'other' && protocols.other_methods && (
           <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
              {protocols.other_methods.map((method: any, idx: number) => (
                 <div key={idx} className="bg-pink-900 border border-pink-400 rounded-xl p-6">
                    <h3 className="text-lg font-bold text-white mb-2">{method.name}</h3>
                    <p className="text-sm text-white mb-4">{method.description}</p>
                    {method.indication && (
                    ...
                    <div className="flex flex-wrap gap-2">
                       {method.points.map((p: string, i: number) => (
                          <span key={i} className="px-3 py-1 bg-pink-950 rounded border border-pink-400 text-sm text-white">
                             {p}
                          </span>
                       ))}
                    </div>
        ...
        {activeTab === 'herbs' && (
          <div className="max-w-3xl mx-auto animate-fade-in space-y-6">
             <div className="bg-pink-900 border border-pink-400 rounded-xl p-6 relative overflow-hidden">
             ...
             <div className="bg-pink-900 border border-pink-400 rounded-xl p-6 relative overflow-hidden">
                ...
                <ul className="text-xs text-white list-disc list-inside space-y-1">
           ...
           <div className="max-w-4xl mx-auto animate-fade-in">
              ...
              <div className="space-y-4">
                 {protocols.research_highlights.map((item: any, idx: number) => (
                    <div key={idx} className="bg-pink-900 border border-pink-400 rounded-xl p-5 hover:border-indigo-500 transition-colors">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                             <span className="text-[10px] text-white uppercase font-bold">Mekanisme</span>
                             <p className="text-sm text-white font-medium">{item.mechanism}</p>
                          </div>
                          <div>
                             <span className="text-[10px] text-white uppercase font-bold">Efek Klinis</span>
           ...
           <div className="max-w-5xl mx-auto animate-fade-in">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 {protocols.case_studies.map((c: any, idx: number) => (
                    <div key={idx} className="bg-pink-900 border border-pink-400 rounded-xl overflow-hidden flex flex-col">
                       <div className="bg-pink-800/80 p-4 border-b border-pink-400">
                          <h3 className="font-bold text-white text-lg">{c.title}</h3>
                          <p className="text-xs text-white italic mt-1">{c.patient_profile}</p>
                       </div>
                       <div className="p-5 space-y-4 flex-1">
                          <div>
                             <span className="text-[10px] text-white uppercase font-bold tracking-wider block mb-1">Riwayat & Diagnosis</span>
                             <p className="text-sm text-white">{c.history}</p>
                             <div className="mt-1 text-sm font-semibold text-tcm-primary">{c.diagnosis}</div>
                          </div>
                          
                          <div className="bg-pink-950 p-3 rounded border border-pink-400">
                             <span className="text-[10px] text-white uppercase font-bold tracking-wider block mb-2">Titik Yang Digunakan</span>
                             <div className="flex flex-wrap gap-2">
                                {c.points_used.map((p: string, i: number) => (
                                   <span key={i} className="text-xs px-2 py-1 bg-pink-800 rounded text-white border border-pink-400">{p}</span>
                                ))}
                             </div>
                          </div>

                          <div>
                             <span className="text-[10px] text-white uppercase font-bold tracking-wider block mb-1">Hasil Follow Up</span>
