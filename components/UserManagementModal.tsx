...
const UserManagementModal: React.FC<Props> = ({ isOpen, onClose, currentUser }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-pink-600/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-pink-900 border border-pink-400 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-pink-400 bg-pink-800 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white">User Management & Access Control</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
           ...
           {/* Add User Form */}
           <div className="bg-pink-950 p-5 rounded-xl border border-pink-400">
              ...
              <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                 <div className="md:col-span-1">
                    <label className="block text-xs font-semibold text-white mb-1">Username</label>
                    <input 
                      type="text" 
                      value={newUsername}
                      onChange={e => setNewUsername(e.target.value)}
                      className="w-full bg-pink-900 border border-pink-400 rounded-lg px-3 py-2 text-sm text-white focus:border-indigo-500 outline-none"
                 ...
                 <div className="md:col-span-1">
                    ...
                    <input 
                      ...
                      className="w-full bg-pink-900 border border-pink-400 rounded-lg px-3 py-2 text-sm text-white focus:border-indigo-500 outline-none font-mono"
           ...
           {/* User List */}
           <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-3">Registered Users ({users.length})</h3>
              <div className="border border-pink-400 rounded-xl overflow-hidden">
                 <table className="w-full text-sm text-left">
                    <thead className="bg-pink-900 text-white font-medium">
                       <tr>
                    ...
                    <tbody className="divide-y divide-slate-800 bg-pink-900/50">
                       {users.map(u => (
                          <tr key={u.username} className="hover:bg-pink-800/80 transition-colors">
                             <td className="px-4 py-3 font-semibold text-white flex items-center gap-2">
                                <User className={`w-4 h-4 ${u.role === 'admin' ? 'text-amber-400' : 'text-white'}`} />
                             ...
                             <td className="px-4 py-3">
                                <span className={`text-xs px-2 py-1 rounded-full uppercase font-bold tracking-wide ${u.role === 'admin' ? 'bg-amber-900/30 text-amber-500 border border-amber-900/50' : 'bg-pink-800 text-white border border-pink-400'}`}>
                                   {u.role}
                                </span>
                             </td>
                             <td className="px-4 py-3 font-mono text-white">{u.password}</td>
