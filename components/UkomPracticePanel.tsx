...
const UkomPracticePanel: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-pink-950 animate-fade-in overflow-hidden">
      {/* Header */}
      <div className="p-6 bg-pink-900 border-b border-pink-400">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-white">Latihan Soal & Pembahasan Uji Kompetensi Nasional</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input 
              ...
              className="w-full bg-pink-950 border border-pink-400 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white focus:border-indigo-500 outline-none transition-all"
            />
          </div>
          <div className="w-full md:w-64 relative">
             ...
             <select 
               ...
               className="w-full bg-pink-950 border border-pink-400 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white appearance-none cursor-pointer outline-none focus:border-indigo-500"
             >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        {filteredQuestions.length > 0 ? (
          filteredQuestions.map((q) => (
            <div key={q.id} className="bg-pink-900 border border-pink-400 rounded-2xl overflow-hidden shadow-lg transition-all hover:border-pink-400">
              <div className="p-5">
                ...
                <p className="text-white leading-relaxed font-medium mb-5">
                   {q.question}
                </p>
                ...
                {q.options && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-6">
                     {q.options.map((opt, i) => (
                       <div key={i} className="bg-pink-950 border border-pink-400 p-3 rounded-xl text-sm text-white">
                          {opt}
                       </div>
                     ))}
                  </div>
                ...
                {visibleAnswers[q.id] && (
                  <div className="animate-fade-in space-y-4 pt-4 border-t border-pink-400">
                ...
                {!visibleAnswers[q.id] && (
                  <button 
                    onClick={() => toggleAnswer(q.id)}
                    className="w-full py-3 bg-pink-950 hover:bg-indigo-900/20 text-white hover:text-indigo-400 rounded-xl text-xs font-bold uppercase tracking-widest border border-pink-400 border-dashed transition-all"
                  >
      ...
      <div className="p-4 bg-pink-900 border-t border-pink-400 text-center">
         <p className="text-[10px] text-white uppercase font-mono tracking-widest">
